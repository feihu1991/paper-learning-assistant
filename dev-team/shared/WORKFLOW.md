...
3. **测试用例评审** - 在测试前评审测试用例的完整性和有效性
   - 检查测试用例覆盖率（功能覆盖、边界条件、异常处理）
   - 验证测试数据的正确性
   - 确认测试环境一致性
   - 评审测试脚本的可维护性
4. **执行测试** - 运行单元测试、集成测试
5. **UI自动化测试** - 使用Playwright/Cypress进行界面测试
6. **代码质量检查** - ESLint、代码规范检查
7. **性能测试** - 响应时间、内存使用测试
8. **生成报告** - 测试报告保存到 `/dev-team/shared/output/qa/`
9. **验收确认** - 标记任务为完成或提交 Bug
